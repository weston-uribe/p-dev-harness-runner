...
const LOCK_FILE = "workflow-model-sync.lock";
const LOCK_STALE_MS = 30_000;
const LOCK_ACQUIRE_TIMEOUT_MS = 5_000;
const LOCK_RETRY_MS = 50;

export interface WorkflowModelSyncLockMetadata {
  ownerId: string;
  claimedAt: string;
}

export interface WorkflowModelSyncLockHandle {
  ownerId: string;
  lockPath: string;
...
async function readLockMetadata(
  filePath: string,
): Promise<WorkflowModelSyncLockMetadata | null> {
  try {
    const raw = await readFile(filePath, "utf8");
    return JSON.parse(raw) as WorkflowModelSyncLockMetadata;
  } catch {
    return null;
  }
}
...
export async function acquireWorkflowModelSyncLock(
  cwd?: string,
): Promise<WorkflowModelSyncLockHandle> {
  const paths = resolveLocalFilePaths(cwd);
  ...
  while (Date.now() < deadline) {
    try {
      ...
      const metadata: WorkflowModelSyncLockMetadata = {
        ownerId,
  ...
  throw new Error("Timed out waiting for workflow model sync lock.");
}

export async function withWorkflowModelSyncLock<T>(
  cwd: string | undefined,
  fn: () => Promise<T>,
): Promise<T> {
  const lock = await acquireWorkflowModelSyncLock(cwd);
  try {
    return await fn();
  } finally {
    await lock.release();
