...
import {
  applySettingsConfigPatchRemote,
  ...
  try {
    ...
    if (!body.confirmed) {
      return NextResponse.json(
        { error: "Settings config writes require explicit confirmation." },
        { status: 400 },
      );
    }

    const result = await applySettingsConfigPatchRemote({
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Settings config apply failed.",
      },
      { status: 400 },
    );
  }
